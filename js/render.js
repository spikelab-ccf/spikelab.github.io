...
function renderFooter(siteData) {
    ...
    if (isNonEmptyString(footer.sourceCode) && footer.licenseText.includes(linkText)) {
...
function renderNav(data) {
    ...
    const sections = ["about", "research", "people", "publications", "photos", "contact"];
...
function renderResearchSection(content) {
    ...
    items.forEach((item, index) => {
        ...
        if (isNonEmptyString(item.name)) {
            textWrap.appendChild(el("h3", { className: "research-name", text: item.name }));
        }
...
function renderPeopleSection(content) {
    ...
    const iconMap = [
        ...
        { key: "scholar", label: "Google Scholar", src: "assets/icons/google-scholar.svg" },
    ...
    for (const group of content.groups ?? []) {
        ...
        for (const member of group.members ?? []) {
            ...
            if (isNonEmptyString(member.name)) {
                headerRow.appendChild(el("h4", { className: "person-name", text: member.name }));
            }
            ...
            if (isNonEmptyString(member.role)) {
                card.appendChild(el("p", { className: "person-role", text: member.role }));
            }
            ...
            if (isNonEmptyString(member.affiliation)) {
                card.appendChild(el("p", { className: "person-affiliation", text: member.affiliation }));
            }
